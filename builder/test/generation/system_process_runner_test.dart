...
''');
  ...
  ///
  /// 이 감시견을 만들게 한 실물은 상류 `flutter_tweakcn_generator` 였다.
  /// Google Fonts API 가 200 이 아닐 때 응답 본문을 비우지 않고 던져서
  /// (`font_downloader.dart:289-303`, 0.4.0 기준) CLI 가 테마 파일을 쓰고
  /// 요약까지 다 출력한 뒤 영원히 살아 있었다. 실측:
  /// `--font-sans: "Segoe UI"` (Google Fonts 에 없어 400) 로 60초 넘게 안 죽었다.
  /// 정상 실행은 1초 미만이다.
  ///
  /// **그 결함은 0.5.0 에서 고쳐졌다** — 응답을 끝까지 읽어야 연결이 풀리도록
  /// 바뀌었고, 모든 읽기에 기한이 붙었다 (kihyun1998/flutter_tweakcn_generator#23).
  /// 그러니 이 테스트는 더 이상 특정 상류 버그의 회귀 테스트가 아니다.
  /// 그대로 두는 이유는 감시견이 애초에 그 CLI 전용이 아니기 때문이다 —
  /// `SystemProcessRunner` 는 `flutter`·`dart`·`build_runner` 를 전부 돌리고,
  /// 아래 손자 프로세스 시나리오는 상류와 아무 상관 없이 `runInShell: true`
  /// 하나에서 나온다. 재현 대상이 결함에서 **모양**으로 바뀐 것이다.
  ///
