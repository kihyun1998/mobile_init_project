...
class ApplicationId {
  ...
  /// 틀린 것을 가르지도 않는다 — 둘 다 "아직 보여줄 것이 없다" 이다. 여기서
  /// 무엇이 잘못됐는지까지 말하면 **막는 자리가 화면 곳곳으로 흩어진다.**
  ///
  /// **그래서 이 둘을 가르는 일은 이름 칸이 맡는다.** 폼은 `PackageName.parse`
  /// 를 따로 불러 그것이 던진 문장을 그 칸에 그대로 옮긴다 — 이 타입이 null
  /// 을 돌려준 이유를 되짚는 것이 아니라, 판정자에게 직접 묻는다. 두 경로가
  /// 같은 값 타입 하나를 보므로 어긋날 수가 없다 (#45).
