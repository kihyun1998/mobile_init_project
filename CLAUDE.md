...
- shadcn 컴포넌트 15개는 `lib/ui/components/` 에 있고 **provider·l10n·navigation 을 물지 않는다.** 끌어들이는 순간 빌더 미리보기가 깨진다 — 이게 격리의 이유다. `app_bottom_nav_bar.dart` 는 이 디렉토리에 있지만 shadcn 컴포넌트가 아니고 l10n·navigation 을 문다.
  ...
  - **순수 UI 패키지를 무는 것도 된다** (`shadcn_checkbox` → `flutter_checkbox`, `shadcn_select` → `flutter_dropdown_button`). 조건은 **색을 우리가 주입할 수 있어야 한다**는 것이다 — 패키지가 자기 테마로 색을 정하면 그 매핑이 미리보기와 생성 결과를 가르는 새 표면이 되므로 쓰지 않는다.
  - **"안 넘긴 색이 어디서 오는가" 는 패키지마다 다르고, 그 답이 작업량을 가른다.** `flutter_checkbox` 는 `theme.colorScheme.primary` 파생이라 안 넘겨도 결국 붙여넣은 CSS 에서 온다. `flutter_dropdown_button` 은 `ThemeData` 의 **레거시 필드**(`splashColor`·`hoverColor`·`hintColor`·`iconTheme.color`·`disabledColor`·`highlightColor`)를 읽는데 `TweakcnTheme` 는 `colorScheme` 과 `extensions` 만 채우므로 **안 넘기면 Material 기본색이 조용히 들어온다.** 후자면 **도달 가능한 색 슬롯을 전부 명시적으로 채우고, 안 채운 것은 도달 불가 근거를 `file:line` 으로 doc-comment 에 적는다** (`shadcn_select.dart` 가 그 형식이다). 빠뜨린 색은 컴파일도 테스트도 통과하고 화면만 다르다.
  - **밝은 테마만 보고 매핑을 확인하지 말 것.** 기본 CSS 는 `background`/`card`/`popover` 가 같은 값이고 `ThemeData.primaryColor` 는 **dark 에서만** `colorScheme.surface` 로 떨어진다. light 에서만 재면 틀린 매핑이 맞아 보인다 (#31 이 실제로 그렇게 속았다). 매핑 테스트는 dark 를 함께 본다.
