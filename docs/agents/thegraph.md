...
| `emptyHomeScreenSource` | 387 | 예제를 끌 때 써 넣는 홈 화면 **전문**. **이 문자열이 생성물로 나간다** — 사용자가 읽는 유일한 자리다. **여기에는 개수를 적지 않는다** (#49): 아무 게이트도 그 값을 안 보므로 검사할 수 없고, 실제로 "shadcn 13종" 이 적힌 채 오래 나갔다. `builder/test/component_count_test.dart` 가 숫자가 돌아오는 것을 잡는다 |
...
- **#51 — 손으로 적은 잡 개수는 그 개수를 늘리는 변경이 조용히 틀리게 만든다.**
  `gates.yml` 이 위 #16 사고를 *"4개 잡이 전부 초록이었다"* 로 적고 있었는데,
  **그때는 이 워크플로가 아예 없었다**(첫 커밋 `31347f5`, #17). 티켓은 그 한 자리를
  보고했지만, 실제로는 **같은 파일에 같은 종류의 오류가 하나 더** 있었다 — 21줄
  아래 `:84` 가 현재 시제로 *"네 잡 전부 초록으로"* 라고 적는데 지금은 **다섯**이다
  (`gh pr checks` 로 실측). 하필 그 문장이 개수를 늘린 `codegen` 잡 **안**에 있다.
  옆 주석에서 숫자를 옮겨 적은 것이다. → 교훈 둘. (1) **고칠 것은 "4를 5로" 가
  아니라 아무도 검사하지 않는 숫자를 말하는 것 자체다.** 둘 다 숫자를 빼는 쪽으로
  고쳤다. (2) **티켓이 보고한 자리만 고치고 지나가지 않는다** — 파일의 검증 가능한
  주장을 전부 훑었고(나머지 셋은 참), 그래서 열거가 끝났다고 말할 수 있다.
  같은 모양의 전례가 `spine` 의 #24 다.
- **#49 — 같은 병이 생성물로 나가는 문장에 있었다. 그리고 반박 렌즈가 *내 원인
  설명*을 깼다.** `emptyHomeScreenSource` 안의 "shadcn 13종" 이 예제를 끈 모든
  생성물의 `home_screen.dart` 로 나갔다 — 사용자가 읽는 유일한 자리이고 아무
  게이트도 안 본다. 여기까지는 #51 과 같다. **다른 것은 그다음이다.** 나는
  `03e43d1` 을 재구성해 *"저자가 디렉토리 파일을 전부 세서(shadcn 12 +
  `app_bottom_nav_bar.dart`) 13 이 나왔다 — 쓰인 날 이미 틀렸다"* 고 결론짓고
  그 서사를 **doc-comment 둘과 테스트 어서션에 사실처럼 박았다.** 반박 렌즈가
  정합적인 대안을 냈다 — 당시 공개 위젯 클래스가 14개인데 스위치의 라벨 변형을
  빼면 **13종**이고, 커밋이 쓴 단어가 "개" 가 아니라 **"종"** 이다. 재현해보니
  맞다. → 교훈 셋. (1) **원인 재구성은 측정이 아니다.** 숫자가 틀렸다는 것은
  쟀지만 *왜* 틀렸는지는 안 쟀는데, 후자가 기록으로 굳고 있었다. (2) 그래서
  진짜 결함은 "숫자가 틀렸다" 가 아니라 **"단위가 어디에도 없어서 검사할 수
  없다"** 다 — 그 진단이 고칠 것을 정한다(단위를 못박고 테스트를 붙인다).
  (3) **회귀 테스트가 표현을 열거하면 회귀가 아니라 과거 문자열을 잡는다.**
  처음 쓴 `shadcn N종|컴포넌트 N개` 는 조사 하나(`컴포넌트가 15개`)나 어순
  (`15개의 shadcn 컴포넌트`)만 바뀌어도 새 나갔고, 결함을 되살린 변이가 3/3
  초록으로 지나갔다. 그 리터럴에는 숫자가 한 글자도 없으므로 `RegExp(r'\d')`
  가 정확하고 우회 불가다.
