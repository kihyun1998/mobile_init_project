...
말한다.** Step 5 렌즈 브리프에 tie-breaker 와 **함께** 실어야 하는 것이다. 안 실으면
렌즈는 자기 소견을 대조할 기록을 못 받은 채로 판정하게 되고, 아래 항목들이 매번
`DELIBERATE` + 인용이 아니라 **`CONFIRMED` + 제안**으로 다시 올라온다. 가정이 아니다 —
1번 항목에서 실제로 그렇게 상류 이슈를 올렸다가 기각됐다.

| 자리 | 무엇이 갈리나 | 기록 |
|---|---|---|
| **radius 파생식** | 우리 생성기는 뺄셈(`base -4 / -2 / +4`), shadcn/ui `globals.css:50-53` 은 곱셈(`×0.6 / 0.8 / 1.4`). 두 식은 `--radius: 10px` 에서만 일치한다 | **#23.** 그 뺄셈은 **tweakcn 자신의 것**이고(tweakcn 이 내보내는 `globals.css` 의 `:174-177`·`:256-259`) 사용자가 복사해오는 CSS 가 그렇게 계산하므로 이 층을 지배한다. 결함으로 읽고 상류에 올렸다가 **기각됐다** (`kihyun1998/flutter_tweakcn_generator#31`, 0.5.1 이 식을 그대로 두고 근거를 doc-comment 로 못박았다) |
| **라디오 그룹의 타일** | shadcn `radio-group.tsx` 에 타일이 없다. 우리는 옵션마다 테두리 상자를 두른다 | **#25** (`shadcn_radio_group.dart:170`). 그래서 그림자는 **표시기**가 받고 타일은 안 받는다. "타일에 `shadow-xs` 가 없다" 는 결함이 아니다 |
| **날짜 선택 오버레이** | shadcn 은 팝오버(`popover.tsx:33`, `shadow-md`), 우리는 `Dialog` 다 | **#25** (`shadcn_date_picker.dart:227`). 그래서 `shadow-lg` 를 쓴다(`dialog.tsx:64`). 단계가 틀린 것이 아니라 **자리가 다른 것**이다 |

**Material 의미론과 갈리는 것은 여기 다시 적지 않는다** (`input → outlineVariant`,
`card → surfaceContainerLowest`). 위 tie-breaker 문단이 이미 다루고, 그건 개별 예외가
아니라 "생성기를 따라간다" 는 규칙 자체다. 여기 옮겨 적으면 규칙이 예외 목록으로
바래고 두 곳이 갈린다.

**목록이 짧은 것이 이 절이 없어도 된다는 뜻은 아니다.** 짧아서 값이 있다 — 셋뿐이면
렌즈에 통째로 실어줄 수 있다. 새 항목이 생기면 그 이슈를 닫는 변경에서 여기에
더한다. 반대로 갈림이 **해소되면 지운다** — select 팝오버 그림자가 그랬다(#31 이
일부러 꺼둔 것을 #25 가 되살렸다).
...
- **#26 의 switch — 형제에서 플래그 이름을 유추하면 틀린다.** 라디오 바로 다음에 한
  작업이라 `checked` 를 그대로 가져다 쓸 뻔했는데, Material 과 Cupertino 둘 다
  스위치에는 **`toggled`** 를 쓴다(`material/switch.dart:1074`,
  `cupertino/switch.dart:753`). 변이로 확인했다 — `toggled` 를 `checked` 로 바꾸면
  5개가 빨개진다. **"바로 옆 컴포넌트에서 방금 배웠다" 가 소스를 안 읽어도 되는
  이유가 아니다.** 반대로 라디오에 있던 플랫폼 분기는 스위치에 **없다**(Cupertino 쪽
  `defaultTargetPlatform` 분기는 햅틱용이다) — 이것도 읽어서 확인한 것이지,
  "형제가 그랬으니" 로 옮겼으면 쓸데없는 `variant:` 장치를 달았을 것이다.
- **`matchesSemantics` 는 자식 노드를 안 본다.** 같은 건에서 걸렸다. 라벨 붙은
  스위치가 안쪽에 컨트롤을 하나 더 들고 있으면 노드가 갈리는데, 바깥 노드에
  `matchesSemantics` 를 걸어도 **통과한다.** "안쪽을 되돌리는" 변이가 1개밖에 안
  잡아서 드러났고, `node.childrenCount` 를 함께 보게 고치니 2개가 잡힌다.
  → 변이 검사가 **테스트 이름이 주장하는 것과 실제로 재는 것의 차이**를 잡은 사례.
